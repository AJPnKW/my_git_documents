const storageKey = "andrewdSupportRoadmapDynamicV2";
...
function labelFor(value, fallback = "Not answered"){
  const labels = {
    on_odsp:"On ODSP", thinks_on_odsp:"Think I am on ODSP", not_on_odsp:"Not on ODSP", unknown:"Not sure",
    yes:"Yes", no:"No",
    ...
    has_passport:"Have Passport funding", pending:"Pending / waiting", none:"None",
...
function clearAnswers(){
  ...
  updateDashboard();
  byId("roadmapSummary").innerHTML = "Answers cleared. Complete the Inquiry again.";
  resetTables();
}
...
function updateDashboard(){
  ...
  byId("readinessText").textContent = chips.length >= 4 ? "Roadmap can be generated from the saved inquiry answers." : "Complete more inquiry fields to improve the roadmap.";
}

function resetTables(){
  byId("pathwayTable").querySelector("tbody").innerHTML = row(["—","No roadmap yet","Complete inquiry first.","Use the Inquiry tab.","—"]);
...
function generateRoadmap(switchTab = true){
  ...
  if(docsWeak){
    steps.push(["6","Build document package","Applications and accommodations can stall without documentation.","Gather ODSP info, assessments, school records, college records, resume, and job-loss history.","Personal prep step"]);
  ...
  steps.push(["7","Accommodation wording","Support works better when functional needs are clear.","Create a short list: what is hard, what helps, what support is needed, and what has failed before.","Employment advisor / Accessible Learning / HRLSC if needed"]);

  byId("pathwayTable").querySelector("tbody").innerHTML = steps.map(s => row(s)).join("");
  ...
  byId("generatedScript").textContent = [
    "Call script:",
    "",
    "I am trying to connect with the right supports. The main issue may not only be finding work; it may be keeping work and having the right support after starting.",
  ...
  byId("roadmapSummary").innerHTML = `<strong>Roadmap generated.</strong> Likely priority: ${jobRetentionNeed ? "job-retention support plus ODSP/DSO/Passport coordination." : "confirm existing supports and build a structured employment/education plan."}`;
  updateDashboard();
